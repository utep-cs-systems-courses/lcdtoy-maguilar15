 ...
 * Custom Struct
 */
typedef struct DemoShape_s {
    void (*getBounds)(const struct DemoShape_s *shape, const Vec2 *centerPos, Region *bounds);
    int (*check)(const struct DemoShape_s *shape, const Vec2 *centerPos, const Vec2 *pixelLoc);
} DemoShape;
